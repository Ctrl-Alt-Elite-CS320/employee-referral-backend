...
exports.applications_all_get = async (req, res) => {
  res.send(
    "not implemented: show applications for position w/ given id:" +
      req.params.id
  );
};
...
exports.application_detail_get = async (req, res) => {
  res.send(
    "not implemented: show application with id:" +
      req.params.appId +
      " for position w/ given id:" +
      req.params.id
  );
};
...
exports.new_post = async (req, res) => {
  res.send("not implemented: post form to create new position");
};
...
exports.new_application_post = [
  ...
  async (req, res, next) => {
    ...
    if (Number.isFinite(posId) || /^\d+$/.test(posId)) {
      ...
      if (results.rows[0].exists != "t") {
        res
          .status(400)
          .send("the job posting you're looking for doesn't exist");
        return;
      }
    } else {
  ...
  body(
    "candDescription",
    "Give a brief description of why the candidate is a good fit"
  )
    .trim()
    .isLength({ min: 1 })
    .escape(),
...
exports.delete_get = async (req, res) => {
  res.send(
    "not implemented: get congirmation for deleting position w/ given id:" +
      req.params.id
  );
};
...
exports.application_delete_get = async (req, res) => {
  res.send(
    "not implemented: get confirmation page or deleting application with id:" +
      req.params.appId +
      " for position w/ given id:" +
      req.params.id
  );
